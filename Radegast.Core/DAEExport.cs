...
namespace Radegast
{
    public class DAEExport : IDisposable
    {
        ...
        public DAEExport(IRadegastInstance instance, Primitive requestedPrim)
        {
            Instance = instance;
            ImageFormat = "PNG";
        ...
        private static bool IsFullPerm(Permissions Permissions)
        {
            return ((Permissions.OwnerMask & PermissionMask.Modify) != 0) &&
                   ((Permissions.OwnerMask & PermissionMask.Copy) != 0) &&
                   ((Permissions.OwnerMask & PermissionMask.Transfer) != 0);
        }

        private static bool IsFullPerm(InventoryItem item)
        {
        ...
        private bool CanExport(Primitive prim)
        {
        ...
        private void AddSource(XmlNode mesh, string src_id, string param, List<float> vals)
        {
            ...
            {
                src_array.Attributes.Append(Doc.CreateAttribute("id")).InnerText =
                    $"{src_id}-array";
        ...
        private void AddPolygons(XmlNode mesh, string geomID, string materialID, FacetedMesh obj, List<int> faces_to_include)
        {
            ...
            // Normals semantic
            {
                ...
                if (input.Attributes != null)
                {
                    ...
                    input.Attributes.Append(Doc.CreateAttribute("offset")).InnerText = "0";
                    input.Attributes.Append(Doc.CreateAttribute("source")).InnerText =
                        $"#{geomID}-normals";
                }
            }
            ...
            // UV semantic
            {
                ...
                if (input.Attributes != null)
                {
                    ...
                    input.Attributes.Append(Doc.CreateAttribute("offset")).InnerText = "0";
                    input.Attributes.Append(Doc.CreateAttribute("source")).InnerText = $"#{geomID}-map0";
                }
            }
        ...
        private readonly List<MaterialInfo> AllMeterials = new List<MaterialInfo>();
        ...
        private List<MaterialInfo> GetMaterials(FacetedMesh obj)
        {
        ...
        private List<int> GetFacesWithMaterial(FacetedMesh obj, MaterialInfo mat)
        {
        ...
        private void GenerateEffects(XmlNode effects)
        {
        ...
        private void GenerateCollada()
        {
            ...
            if (scene.Attributes != null)
            {
                ...
                foreach (var obj in MeshedPrims)
                {
                    ...
                    {
                        AddPolygons(mesh, geomID, objMaterial.Name + "-material", obj,
