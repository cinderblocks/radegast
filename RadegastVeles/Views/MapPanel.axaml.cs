...
public partial class MapPanel : UserControl
{
    ...
    protected override void OnLoaded(RoutedEventArgs e)
    {
        ...
        // Refresh online friends when panel opens
        _vm.RefreshOnlineFriends();

        // Focus map on current simulator position
    ...
    {
        if (_vm == null) return;
        _vm.SetMapTarget(e.RegionGridX, e.RegionGridY, e.LocalX, e.LocalY);
    }

    private void OnMapDoubleClicked(object? sender, MapClickEventArgs e)
    {
        _vm?.TeleportToPosition(e.RegionGridX, e.RegionGridY, e.LocalX, e.LocalY);
    }

    private bool _suppressZoomSync;

    private void OnMapZoomChanged(object? sender, double newZoom)
    {
        if (_vm == null || _suppressZoomSync) return;
        _suppressZoomSync = true;
        _vm.ZoomLevel = Math.Clamp((newZoom - 0.5) / 9.5 * 100.0, 0, 100);
        _suppressZoomSync = false;
    }

    private void OnVisibleRangeChanged(object? sender, VisibleRangeEventArgs e)
    {
        _vm?.RequestMapBlocksForRange(e.MinX, e.MinY, e.MaxX, e.MaxY);
    }

    private void OnVmPropertyChanged(object? sender, PropertyChangedEventArgs e)
    {
        if (e.PropertyName == nameof(MapViewModel.ZoomLevel) && _vm != null && _mapControl != null && !_suppressZoomSync)
        {
            double zoom = 0.5 + (_vm.ZoomLevel / 100.0) * 9.5;
