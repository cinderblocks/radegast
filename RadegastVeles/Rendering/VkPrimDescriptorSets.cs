...
{
    public DescriptorSet FrameSet { get; }
    public DescriptorSet PassSet { get; }

    private readonly VkContext _vk;
    private readonly Buffer _frameUboBuffer;
    private readonly DeviceMemory _frameUboMemory;
    ...
    public VkPrimDescriptorSets(VkContext vk, VkPrimPipeline pipeline, VkPlaceholderTextures placeholders)
    {
        ...
        VkBufferHelper.AllocateHostVisible(vk, BufferUsageFlags.UniformBufferBit,
            out _frameUboBuffer, out _frameUboMemory, MemoryMarshal.CreateReadOnlySpan(ref initialFrame, 1));

        FrameSet = AllocateSet(vk, pipeline.PerFrameLayout);
        WriteFrameSet();
    ...
    /// <summary>Overwrites the per-frame UBO's contents -- call once per rendered frame before
    /// recording draw commands (host-visible/coherent memory, no descriptor rewrite needed,
    /// only the buffer's bytes change).</summary>
    public void UpdatePerFrame(in VkPerFrameUbo data)
    {
        VkBufferHelper.UpdateHostVisible(_vk, _frameUboMemory, MemoryMarshal.CreateReadOnlySpan(ref Unsafe.AsRef(in data), 1));
    }
    ...
    public void Dispose()
    {
        if (_disposed) return;
        _disposed = true;
        var sets = stackalloc DescriptorSet[2] { FrameSet, PassSet };
        _vk.Api.FreeDescriptorSets(_vk.Device, _vk.DescriptorPool, 2, sets);
        _vk.Api.DestroyBuffer(_vk.Device, _frameUboBuffer, null);
        _vk.Api.FreeMemory(_vk.Device, _frameUboMemory, null);
    }
}
