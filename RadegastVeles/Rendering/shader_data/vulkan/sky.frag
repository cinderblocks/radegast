...
// Vulkan variant of ../sky.frag. See plan Section 7/8c-2 -- function bodies below the two
// uniform blocks are byte-identical to the GL original; only the uniform declarations changed.
//
// Set 0 is the SAME PerFrame UBO prim.vert/prim.frag declare (this pipeline's layout reuses
// VkPrimPipeline.PerFrameLayout directly -- one shared VkDescriptorSetLayout object, one shared
// _frameSets.FrameSet buffer, bound at set 0 for both the main geometry pass and this one). The
// full field list must be declared here even though only the atmosphere-related fields are read,
// because std140 offsets are positional: this shader reads uBlueHorizon etc. through the SAME
// physical buffer bytes prim.vert/prim.frag's own copy of this struct maps them to, and getting
// those offsets right requires declaring every preceding field, not just the ones this shader
// happens to use.
...
// Set 1: sky-pass-only data -- not shared with any other pipeline (unlike set 0), since nothing
// else needs uInvViewProj/cloud parameters/uCloudNoise. See VkSkyUbo.cs for the C#-side mirror
// and its own std140-offset verification note.
layout(set = 1, binding = 0) uniform SkyPass
{
    mat4  uInvViewProj;    // inverse(view * proj) -- reconstructs world-ray
    vec3  uCloudColor;     // WL cloud_color
    float uCloudScale;     // WL cloud_scale
    vec4  uCloudPosDensity1; // WL cloud_pos_density1 (xy=offset, w=coverage)
    vec4  uCloudPosDensity2; // WL cloud_pos_density2 (detail octave)
    vec2  uCloudScrollRate;  // WL cloud_scroll_rate
    float uCloudShadow;      // WL cloud_shadow
    float uCloudVariance;    // EEP cloud_variance
    vec3  uCameraPos;        // world-space eye position
    float uTime;             // seconds, drives cloud scroll animation
} skyPass;
layout(set = 1, binding = 1) uniform sampler2D uCloudNoise; // tiling value-noise density texture (R8, mipmapped)

// Aliased through "skyPass", NOT "sky" -- the GL original's own main() declares a local
// `vec3 sky` (the accumulated sky colour being composited), which would otherwise shadow a
// UBO instance literally named `sky` and turn e.g. `sky.uCloudShadow` into an illegal swizzle
// on that local vec3 (found empirically via dotnet-shaderc: "vector swizzle too long").
#define uInvViewProj      skyPass.uInvViewProj
#define uCloudColor       skyPass.uCloudColor
#define uCloudScale       skyPass.uCloudScale
#define uCloudPosDensity1 skyPass.uCloudPosDensity1
#define uCloudPosDensity2 skyPass.uCloudPosDensity2
#define uCloudScrollRate  skyPass.uCloudScrollRate
#define uCloudShadow      skyPass.uCloudShadow
#define uCloudVariance    skyPass.uCloudVariance
#define uCameraPos        skyPass.uCameraPos
#define uTime             skyPass.uTime
...
float sampleCloudLayer(int i, vec3 dir, out float planeDist, out float sunShade)
{
    ...
    float lod2 = min(lod + 1.2, 3.0); // detail octave tiles 2.3x denser
