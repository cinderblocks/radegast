...
// Vulkan variant of ../prim.frag. Only the declaration block below (through the #define
// aliases) differs from the GL original -- every function body from `vec2
// transformUv(...)` onward is byte-identical to it, so the shading/PBR/terrain-triplanar
// math itself is untouched, only how its inputs are declared. This keeps the highest-risk
// part of this file (700+ lines of precision-sensitive lighting math) out of the
// conversion's blast radius entirely.

layout(location = 3) in vec3 vNormal;
layout(location = 4) in vec3 vViewPos;
layout(location = 5) in vec2 vTexCoord;
layout(location = 7) in vec3 vObjPos;
layout(location = 8) in vec3 vObjNormal;
layout(location = 9) in vec3 vWorldPos;

layout(location = 0) flat in vec4 vInstColor;
layout(location = 1) flat in vec4 vInstMisc;
layout(location = 2) flat in int  vInstAlphaMode;
layout(location = 6) in vec4 vTangent;

layout(location = 0) out vec4 fragColor;

// Set 0: per-frame data -- see prim.vert for the full struct (shared descriptor set, must
// match exactly). #define-aliased below so the shading code can keep reading these as bare
// names, same technique vulkan/shadow.glsl and vulkan/atmosphere.glsl already use.
...
#define uSunDir       frame.uSunDir
#define uSunColor     frame.uSunColor
#define uAmbientColor frame.uAmbientColor
#define uFogDensity   frame.uFogDensity
#define uHasSsao      frame.uHasSsao
#define uScreenSize   frame.uScreenSize
#define uPointLightCount  frame.uPointLightCount
#define uPointLightPos    frame.uPointLightPos
#define uPointLightColor  frame.uPointLightColor
#define uPointLightRadius frame.uPointLightRadius
#define uPointLightFalloff frame.uPointLightFalloff

// Set 1: per-pass samplers (shadow.glsl declares uShadowMap/uPointShadowMap0/1 at
...
layout(set = 2, binding = 5) uniform Material
{
    int  uHasTexture;
    int  uHasBump;
    int  uIsTerrain;
    ...
    vec4 uPbrNormalUvST;
    float uPbrNormalUvRot;
    vec4 uBaseColorUvST;
    float uBaseColorUvRot;
} mat;

#define uHasTexture        mat.uHasTexture
#define uHasBump           mat.uHasBump
#define uIsTerrain         mat.uIsTerrain
#define uHasMaterial       mat.uHasMaterial
#define uHasNormalMap      mat.uHasNormalMap
#define uNormalUvST        mat.uNormalUvST
#define uNormalUvRot       mat.uNormalUvRot
#define uHasSpecularMap    mat.uHasSpecularMap
#define uSpecUvST          mat.uSpecUvST
#define uSpecUvRot         mat.uSpecUvRot
#define uSpecColor         mat.uSpecColor
#define uSpecExp           mat.uSpecExp
#define uEnvIntensity      mat.uEnvIntensity
#define uIsPBR             mat.uIsPBR
#define uHasMRMap          mat.uHasMRMap
#define uMRUvST            mat.uMRUvST
#define uMRUvRot           mat.uMRUvRot
#define uHasEmissiveMap    mat.uHasEmissiveMap
#define uEmissiveUvST      mat.uEmissiveUvST
#define uEmissiveUvRot     mat.uEmissiveUvRot
#define uBaseColorFactor   mat.uBaseColorFactor
#define uMetallicFactor    mat.uMetallicFactor
#define uRoughnessFactor   mat.uRoughnessFactor
#define uEmissiveFactor    mat.uEmissiveFactor
#define uPbrNormalUvST     mat.uPbrNormalUvST
#define uPbrNormalUvRot    mat.uPbrNormalUvRot
#define uBaseColorUvST     mat.uBaseColorUvST
#define uBaseColorUvRot    mat.uBaseColorUvRot
...
// by SceneLightStreamer and selected per-frame by the viewport. Colour is
