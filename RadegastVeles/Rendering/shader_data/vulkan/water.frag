...
// Vulkan variant of ../water.frag. Function bodies below the two uniform blocks are the same as
// the GL original except for the gl_FragDepth write, called out at its own site: the same NDC
// depth-range handling used in ssao.frag and shadow.glsl.
//
// Set 0 is the SAME PerFrame UBO prim.vert/prim.frag/sky.frag declare (this pipeline's layout
// reuses VkPrimPipeline.PerFrameLayout directly -- see VkWaterPipeline.cs's own doc comment for
// why the full field list must be declared even though this shader only reads a few of them).
...
// Shared atmosphere model (uSunDirection, uSunlightColor, uAmbient, haze params...) -- reads
// through the PerFrame block above via #define aliasing, same as sky.frag/prim.frag.
#include "atmosphere.glsl"

// Set 1: water-pass-only data -- not shared with any other pipeline, since nothing else needs the
// reflection/inverse-VP matrices or the water-specific scalars. These are computed and uploaded
// CPU-side already-composed rather than composed from frame.uView/frame.uProj in-shader, avoiding
// any row/column-vector composition-order ambiguity for a matrix product (see VkWaterUbo.cs).
layout(set = 1, binding = 0) uniform WaterPass
{
    mat4  uViewProj;       // view * proj -- for gl_FragDepth of the hit point
    mat4  uReflViewProj;   // reflected-camera view * proj -- for reflection UV lookup
    mat4  uInvViewProj;    // inverse(view * proj) -- ray reconstruction
    vec3  uEyePos;         // camera world position (Z-up)
    float uWaterHeight;    // world Z of the water plane
...
#define uInvViewProj   waterPass.uInvViewProj
#define uEyePos        waterPass.uEyePos
#define uWaterHeight   waterPass.uWaterHeight
...
void main()
{
    ...
    if (uHasReflection != 0 && t < kMaxReflDist)
    {
        ...
        // Fade the FBO sample out toward its UV edges and toward kMaxReflDist (where the 512²
        // FBO can no longer resolve the hit) instead of clamping, which smears edge texels into
        // streaky artefacts.
    ...
    //
    // GL's hardware depth-range remap does not apply to an explicit gl_FragDepth write (a shader
    // that writes it is responsible for the final [0,1] value itself), so GL's shader does the
    // *0.5+0.5 conversion manually so this manually-written depth compares correctly against
    // terrain's automatically remapped depth. Vulkan's rasterizer has no such automatic remap for
    // terrain's depth either (native, unscaled [0,1] -- see ssao.frag's viewPosFromDepth), so
    // water's manual write must match that: raw clipHit.z/clipHit.w, no additional scale/bias.
    vec4 clipHit  = uViewProj * vec4(hitPos, 1.0);
    gl_FragDepth  = clamp(clipHit.z / clipHit.w, 0.0, 1.0);
}
