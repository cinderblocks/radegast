...
internal sealed class VkSsaoPipeline : IDisposable
{
    public PipelineLayout Layout { get; }
    public Pipeline Pipeline { get; }
    public DescriptorSetLayout ParamsLayout { get; }

    private readonly VkContext _vk;

    private VkSsaoPipeline(VkContext vk, PipelineLayout layout, Pipeline pipeline, DescriptorSetLayout paramsLayout)
    {
        _vk = vk;
        Layout = layout;
        Pipeline = pipeline;
    ...
    public static unsafe VkSsaoPipeline Create(VkContext vk, RenderPass renderPass)
    {
        var paramsLayout = VkDescriptorSetLayouts.CreateSsaoParamsLayout(vk);

        PipelineLayout layout = default;
        Pipeline pipeline = default;
        try
        {
            CreatePipelineInternal(vk, renderPass, paramsLayout, out layout, out pipeline);
            return new VkSsaoPipeline(vk, layout, pipeline, paramsLayout);
        }
        catch
        {
            var api = vk.Api;
            ...
            api.DestroyDescriptorSetLayout(vk.Device, paramsLayout, null);
            throw;
        }
    }

    private static unsafe void CreatePipelineInternal(VkContext vk, RenderPass renderPass,
        DescriptorSetLayout paramsLayout, out PipelineLayout layout, out Pipeline pipeline)
    {
        var setLayout = paramsLayout;
        ...
        using var frag = VkShaderModule.LoadFromFile(vk, "Rendering/shader_data/vulkan/ssao.frag.spv");
        ...
        // No vertex buffer -- quad.vert generates the full-screen triangle from gl_VertexIndex.
        ...
        // No depth attachment on this render pass at all -- matches GL's SSAO FBO (color-only,
        // Disable(DepthTest), DepthMask(false)).
    ...
    public unsafe void Dispose()
    {
        ...
        api.DestroyDescriptorSetLayout(_vk.Device, ParamsLayout, null);
    }
}
